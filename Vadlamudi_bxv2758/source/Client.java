...
//			ePanel.setPreferredSize(new Dimension(32, 90));//   2*ICONHEIGHT
            ePanel.setPreferredSize(new Dimension(90, 120));//   2*ICONHEIGHT
...
if(name.equals(currentname.substring(0,currentname.indexOf("("))))
{
	...
	if(flag)
      ...
      abort.addMouseListener(new MouseAdapter(){
		...
		{
			myrole="PA";
			//JOptionPane.showMessageDialog(this,"In Participant");
...
out.writeObject(commituser + " posted answer   with  Commit");
 ...
 else if(s.equals("Vote"))
{
	    ...
	    if(myrole.equals("CO"))
	{
                            ...
                            if (out != null)
      	{
			...
			//JOptionPane.showMessageDialog(new JFrame(),end-start);
            ...
            do{
				...
				if(login.getNewUser())
				{
                   ...
                   // n.setVisible(true);
                    ...
                    n.toFront();
					udata=n.retData();
					//JOptionPane.showMessageDialog(this," New Data---> " + udata);
					name = n.getName();
					myname=n.getName();
					strLogin = n.getName();
  ...
  else if(nt.equals("GLOBALABORT"))
{
	...
	//  refreshTime(cnt);
		...
		{  
		 	invalid=false;
  			//appendDisplay(output,"GLOBAL COMMIT DONE");
			...
			for(int i=0;i<answers.size();i++)
				...
				out.writeObject("GCOMM#"+name+"#"+question);
		}		
}
}
 }
